...
//  FavPlacesMaps
//
//  Created by Eichan Nakagawa on 04/07/2025.
//
        ...
        ZStack {
                ...
                Map(position: $position) {
                    ForEach(places) { place in
                        Annotation(place.name, coordinate: place.coordinates) {
                            let color =
                                if place.fav { Color.yellow } else {
                                    Color.black
                                }
                            Circle().stroke(color, lineWidth: 3).fill(.white)
            ...
            ScrollView(.horizontal) {
                LazyHStack {
                    ForEach(places) { place in
                        let color =
                            if place.fav { Color.yellow.opacity(0.5) } else {
...
extension FavPlacesView {
    ...
    func loadPlaces() {
        if let savedPlaces = UserDefaults.standard.data(forKey: "places") {
            if let decodedPlaces = try? JSONDecoder().decode(
                [Place].self,
                from: savedPlaces
            ) {
